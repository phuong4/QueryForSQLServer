...
DECLARE @SearchValue NVARCHAR(100) = N'StringToFind';
...
BEGIN
    ...
    SET @SqlQuery = N'INSERT INTO #SearchResults (TableName, ColumnName)
    SELECT ''' + @TableName + ''', ''' + @ColumnName + ''' WHERE EXISTS 
    (SELECT 1 FROM ' + @TableName + ' WHERE [' + @ColumnName + '] COLLATE Vietnamese_CI_AI 
    LIKE N''%' + @SearchValue + '%'')'
    -- Replace Vietnamese_CI_AI with the collation that matches the Unicode string you want to find
